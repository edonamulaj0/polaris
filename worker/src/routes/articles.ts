...
articlesRouter.get('/', async (c) => {
  ...
  let query = `SELECT * FROM articles WHERE hidden = 0`; // [WRK-3]
  ...
  const articles = slice.map(rowToPublic); // [WRK-3]
...
articlesRouter.get('/:id', async (c) => {
  ...
  const row = await c.env.DB.prepare(`SELECT * FROM articles WHERE id = ? AND hidden = 0`)
    .bind(id)
    .first<ArticleRow>(); // [WRK-3]
